...
int32_t LaneEngine::Initialize(const std::string& work_dir, const int32_t num_threads) {
  ...
  if (inference_helper_->Initialize(model_filename, input_tensor_info_list_,
                                    output_tensor_info_list_) != InferenceHelper::kRetOk) {
    inference_helper_.reset();
    return kRetErr;
  }
...
static std::vector<int32_t> argmax_1(const std::vector<float>& v,
...
static int32_t sum_valid(const std::vector<int32_t>& v, int32_t num, int32_t interval,
                         int32_t offset) {
  int32_t sum = 0;
...
std::vector<LaneEngine::Line<float>> LaneEngine::Pred2Coords(
    const std::vector<float>& loc_row, const std::vector<int32_t>& loc_row_dims,
    const std::vector<float>& exist_row, const std::vector<int32_t>& exist_row_dims,
    const std::vector<float>& loc_col, const std::vector<int32_t>& loc_col_dims,
    const std::vector<float>& exist_col, const std::vector<int32_t>& exist_col_dims) {
    ...
    if (sum_valid(valid_row, num_cls_row, num_lane_row, i) > num_cls_row / 2) {
      for (int32_t k = 0; k < num_cls_row; k++) {
        ...
        if (valid_row[index] != 0) {
          ...
          for (int32_t all_ind = std::max(0, max_indices_row[index] - 1);
               all_ind <= std::min(num_grid_row - 1, max_indices_row[index] + 1); all_ind++) {
          ...
          CommonHelper::SoftMaxFast(pred_all_list.data(), pred_all_list_softmax.data(),
                                    pred_all_list.size());
          float out_temp = 0;
    ...
    if (sum_valid(valid_col, num_cls_col, num_lane_col, i) > num_cls_col / 8) {
      for (int32_t k = 0; k < num_cls_col; k++) {
        ...
        if (valid_col[index] != 0) {
          std::vector<float> pred_all_list;
          std::vector<int32_t> all_ind_list;
          for (int32_t all_ind = std::max(0, max_indices_col[index] - 1);
               all_ind <= std::min(num_grid_col - 1, max_indices_col[index] + 1); all_ind++) {
...
int32_t LaneEngine::Process(const cv::Mat& original_mat, Result& result) {
  ...
  /* do crop, resize and color conversion here because some inference engine doesn't support these
   * operations */
#if defined(USE_CULANE)
  int32_t crop_x = 0;
...
#endif
  cv::Mat img_src =
      cv::Mat::zeros(input_tensor_info.GetHeight(), input_tensor_info.GetWidth(), CV_8UC3);
  CommonHelper::CropResizeCvt(original_mat, img_src, crop_x, crop_y, crop_w, crop_h, IS_RGB,
                              CommonHelper::kCropTypeStretch);
  // CommonHelper::CropResizeCvt(original_mat, img_src, crop_x, crop_y, crop_w, crop_h, IS_RGB,
  // CommonHelper::kCropTypeCut); CommonHelper::CropResizeCvt(original_mat, img_src, crop_x, crop_y,
  // crop_w, crop_h, IS_RGB, CommonHelper::kCropTypeExpand);
  ...
  std::vector<float> loc_row(
      output_tensor_info_list_[0].GetDataAsFloat(),
      output_tensor_info_list_[0].GetDataAsFloat() + output_tensor_info_list_[0].GetElementNum());
  std::vector<float> loc_col(
      output_tensor_info_list_[1].GetDataAsFloat(),
      output_tensor_info_list_[1].GetDataAsFloat() + output_tensor_info_list_[1].GetElementNum());
  std::vector<float> exist_row(
      output_tensor_info_list_[2].GetDataAsFloat(),
      output_tensor_info_list_[2].GetDataAsFloat() + output_tensor_info_list_[2].GetElementNum());
  std::vector<float> exist_col(
      output_tensor_info_list_[3].GetDataAsFloat(),
      output_tensor_info_list_[3].GetDataAsFloat() + output_tensor_info_list_[3].GetElementNum());
  ...
  auto line_list = Pred2Coords(loc_row, loc_row_dims, exist_row, exist_row_dims, loc_col,
                               loc_col_dims, exist_col, exist_col_dims);

  /* todo: I'm not sure the following code correct */
  /* Adjust height scale :
   * https://github.com/cfzd/Ultra-Fast-Lane-Detection-v2/blob/c80276bc2fd67d02579b6eeb57a76cb5a905aa3d/demo.py#L88
   */
  /* It looks the demo code run inference with height = model_input_height / 0.6 . but our code
   * cannot do this. so after running inference with height = model_input_height, adjust y position
   */
  const float kInferenceHeight = input_tensor_info.GetHeight() / kCropRatio;
  for (auto& line : line_list) {
    for (auto& p : line) {
      p.second =
          ((p.second * kInferenceHeight) - (kInferenceHeight - input_tensor_info.GetHeight())) /
  ...
  result.time_pre_process =
      static_cast<std::chrono::duration<double>>(t_pre_process1 - t_pre_process0).count() * 1000.0;
  result.time_inference =
      static_cast<std::chrono::duration<double>>(t_inference1 - t_inference0).count() * 1000.0;
  result.time_post_process =
      static_cast<std::chrono::duration<double>>(t_post_process1 - t_post_process0).count() *
      1000.0;
  ;

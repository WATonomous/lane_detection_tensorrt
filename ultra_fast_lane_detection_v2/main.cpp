...
int32_t main(int argc, char* argv[]) {
  ...
  // writer = cv::VideoWriter("out.mp4", cv::VideoWriter::fourcc('M', 'P', '4', 'V'),
  // (std::max)(10.0, cap.get(cv::CAP_PROP_FPS)),
  ...
  for (frame_cnt = 0; cap.isOpened() || frame_cnt < LOOP_NUM_FOR_TIME_MEASUREMENT; frame_cnt++) {
    ...
    /* Input key command */
    if (cap.isOpened()) {
      /* this code needs to be before calculating processing time because cv::waitKey includes image
       * output */
      /* however, when 'q' key is pressed (cap.released()), processing time significantly
       * incraeases. So escape from the loop before calculating time */
    ...
    if (frame_cnt >
        0) { /* do not count the first process because it may include initialize process */
